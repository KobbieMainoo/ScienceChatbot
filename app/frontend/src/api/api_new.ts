...
export async function uploadImageApi(file: File, idToken: string): Promise<string> {
    const formData = new FormData();
    formData.append("file", file); // 与现有 /upload 期望的字段保持一致
    ...
    if (!response.ok) {
        throw new Error(`Uploading image failed: ${response.statusText}`);
    }

    const dataResponse: SimpleAPIResponse = await response.json();

    // 这里假设后端返回的是 { message: "uploaded/xxx.png" } 或类似内容
    // 你可以根据自己的后端返回结构调整解析逻辑
    if (!dataResponse.message) {
        throw new Error("Image upload response missing 'message'");
    }
