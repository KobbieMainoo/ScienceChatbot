...
import { chatApi, configApi, RetrievalMode, ChatAppResponse, ChatAppResponseOrError, ChatAppRequest, ResponseMessage, SpeechConfig, uploadImageApi} from "../../api"; //uploadImageApi为后加 
...
const Chat = () => {
    console.log("[Chat_new] component rendered");
    ...
    const makeApiRequest = async (question: string, imageFileArg?: File | null) => {
        console.log("makeApiRequest called", question, imageFileArg);
        ...
        try {
            ...
            // 1. 默认用户文本内容就是 question
            const userContent = question;

            // 2. 如果有图片，先上传得到 URL
            let imageAttachments: { url: string; name?: string; mimeType?: string }[] = [];

            // if (imageFileArg && token) {
            if (imageFileArg) {
                // const uploadedPathOrUrl = await uploadImageApi(imageFileArg, token);
                const uploadedPathOrUrl = await uploadImageApi(imageFileArg, token ?? "");

                imageAttachments.push({
                    url: uploadedPathOrUrl,
                    name: imageFileArg.name,
                    mimeType: imageFileArg.type
                });
            }

            const request: ChatAppRequest = {
                messages: [...messages, { content: userContent, role: "user" }],
                context: {
                    overrides: {
                ...
                <div className={styles.chatContainer}>
                    ...
                    <div className={styles.chatInput}>
                        <QuestionInput
                            ...
                            onSend={(question, imageFile) => makeApiRequest(question, imageFile)}
